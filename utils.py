...
from raw_data_preprocessor import load_civil_codes, load_samples, _article_content
...
def micro_result(count_real_lb, count_predicted, count_true,  count_gold_lb=138):
...
def aggregate_all_results(prediction_files, gold_test_file, topk=1, append_unpredicted_q=True):
    ...
    if append_unpredicted_q:
        ...
        for q_id in miss_prediction_keys:
            ...
            predicted_results[q_id] = unpredicted_results[q_id][:1]
    ...
    for q_id, a_info in stats_result.items():
        ...
        if topk is None:
            ...
            for a_id in enssemble_scores:
                enssemble_scores[a_id] =  sum(enssemble_scores[a_id]) / len(enssemble_scores[a_id]) # max(enssemble_scores[a_id]) #
...
def create_df(query_set,civil_dict, neg_sampling=True,top_bm25=10):
  res=[]
  corpus=list(civil_dict.values())
  tokenized_corpus = [doc.split(" ") for doc in corpus]
  bm25 = BM25Okapi(tokenized_corpus)
  for query in query_set:
    #negative sampling (using bm25)
    neg_list=bm25.get_top_n(query["content"].split(" "), corpus, n=top_bm25)
    for idx in query["result"]:
      res.append({
          "content":query["content"],
          "article_content":[sent.strip() for sent in civil_dict[idx].split("\n") if sent.strip()!=""],
          "article_id":idx,
          "label":1
      })
      #remove positive sample from neg=list
      while civil_dict[idx] in neg_list:
        neg_list.remove(civil_dict[idx])
    if neg_sampling: #if it is a train set or dev set
      for i in range(len(neg_list)):
        res.append({
            "content":query["content"],
            "article_content":[sent.strip() for sent in neg_list[i].split("\n") if sent.strip()!=""],
            "article_id":"neg",
            "label":0
        })
